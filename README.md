...
- **商用色卡匹配**：将采集颜色匹配到 RAL 经典色卡（216 色，含官方中文色名）与中国传统色（158 色），显示色号、色名与 ΔE 色差
- **调色配方推荐**：基于减色混合模型，计算达到目标色所需的基色颜料及配比
- **色差评估**：使用 CIEDE2000 色差公式评估配方精度
- **和谐配色方案**：根据色彩理论推荐互补色、类似色、三角色等
- **完整调色报告**：一键生成包含色值、分析、色卡匹配、配方、配色的文字报告
...
| 数据 | 数量 | 来源 | 许可 |
| --- | --- | --- | --- |
| RAL 经典色卡 RGB 值 | 216 | [iw365/RAL-pallete-data](https://github.com/iw365/RAL-pallete-data) | GPL-3.0 |
| RAL 中文色名 | 216 | [劳尔中国官网](http://ralcolours.com.cn/about/allcolor/allcolorth/) | — |
| 中国传统色 | 158 | [longtian/ancient-chinese-color](https://github.com/longtian/ancient-chinese-color) | MIT |

色卡数据由 `tools/gen_paint_library.py` 生成到 `paint_library.py`，可重新生成。
